...
import { loadColumns } from './configureIngredientsCols';
...
const TableSampleIngredients = ({
  ...
  useEffect(() => {
    if (ingredientsNotify.showNotification) {
      notify(
        ingredientsNotify.typeNotification,
        ingredientsNotify.textNotification,
      );
    }
  }, [ingredientsNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/ingredients/ingredients-view/?id=${id}`);
  };
